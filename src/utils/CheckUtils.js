...
export default {
   ...
   * @returns {boolean}
   */
  checkFileInfo(file, rule,onlyShowUpload) {
    ...
    let shop = rule.radio;
    let area = rule.select;
    let isShop = fileName.indexOf(shop.label.toUpperCase()) > -1 || fileName.indexOf(shop.shortName.toUpperCase()) > -1;
    let isArea = fileName.indexOf(area.label.toUpperCase()) > -1 || fileName.indexOf(area.shortName.toUpperCase()) > -1;
    if (!isShop) {
      message.errorMessage(`不是${shop.label}的文件/请注意操作`);
      return false;
    } else if (!isArea) {

      message.errorMessage(`不是${area.label}的文件~`);
      return false
    }else {
      return true
    }
